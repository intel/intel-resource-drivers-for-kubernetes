	...
	testcases := []testCase{
		{
			...
			request: []*resourcev1.ResourceClaim{
				testhelpers.NewClaim(testNameSpace, "claim1", "uid1", "request1", "qat.intel.com", testNodeName, "qat.intel.com", []string{"qatvf-0000-aa-00-1"}, false),
			},
			expectedResponse: map[types.UID]kubeletplugin.PrepareResult{
				"uid1": {
		...
		},
		{
			...
			request: []*resourcev1.ResourceClaim{
				testhelpers.NewClaim("namespace2", "claim2", "uid2", "request2", "qat.intel.com", "node1", "qat.intel.com", []string{"qatvf-0000-aa-00-1"}, false),
			},
			expectedResponse: map[types.UID]kubeletplugin.PrepareResult{
				"uid2": {
					Devices: []kubeletplugin.Device{
		...
		},
		{
			...
			request: []*resourcev1.ResourceClaim{
				testhelpers.NewClaim(testNameSpace, "claim3", "uid3", "request3", "qat.intel.com", testNodeName, "qat.intel.com", []string{"qatvf-xxxx-xx-xx-x"}, false),
			},
			expectedResponse: map[types.UID]kubeletplugin.PrepareResult{
