...
func FuzzGetDefaultVFMemoryFromConfigMap(f *testing.F) {
	f.Add("{\"flex170\":4096}")
	f.Add("{\"flex170\":2048}")
	f.Add("{\"flex170\":16348}")
	f.Fuzz(func(t *testing.T, fileContents string) {
		...
		if resErr == nil {
			if resVal > 16128 || resVal < 854 {
				t.Errorf("unexpected default VF Memory: %v( expected 854 ~ 16128)", resVal)
			}
		}
	})
}
...
func TestGetGpuVFDefaults(t *testing.T) {
	...
	for _, testcase := range testcases {
		vfMem, _, vfProfileName, err := getGpuVFDefaults(testcase.deviceId, testcase.eccOn)
